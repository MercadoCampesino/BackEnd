...
namespace MC_BackEnd.Controllers
{
    ...
    [ApiController]
    public class AutenticacionTiendaController : ControllerBase
    {
        private readonly string secretKey;
        private readonly string cadenaSQL;

        public AutenticacionTiendaController(IConfiguration config)
        {
        ...
        public IActionResult Validar([FromBody] TiendaValidar request)
        {
            ...
            {
                connection.Open();
                string sql = "SELECT COUNT(*) FROM Tienda WHERE correo = @correo AND contrasenia = @contrasenia";
